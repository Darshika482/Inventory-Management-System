...
import { KeyRound, Shield, User as UserIcon, AlertCircle } from 'lucide-react';
import { motion } from 'motion/react';
import { User } from '../types';

interface LoginProps {
  onLogin: (user: User) => void;
  users: Array<{ username: string; role: 'Admin' | 'Worker'; passwordHash: string }>;
}
...
export function Login({ onLogin, users }: LoginProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (matchedUser) {
      ...
      setError('Invalid username/Worker ID or password.');
    }
  };
      ...
      >
        ...
        <div className="p-8 sm:p-10">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="block w-full pl-10 pr-3 py-2.5 bg-white border border-slate-200 rounded-md text-slate-900 placeholder-slate-400 text-xs focus:outline-none focus:border-[#0F172A] focus:ring-1 focus:ring-[#0F172A] transition-all"
                />
              </div>
            </div>
            ...
            <button
              type="submit"
              className="w-full py-2.5 px-4 bg-[#0F172A] hover:bg-slate-800 text-white font-semibold uppercase tracking-wider rounded-md transition-colors focus:ring-2 focus:ring-[#0F172A] focus:outline-none flex items-center justify-center gap-2 cursor-pointer shadow-sm text-xs mt-2"
            >
              Authenticate
            </button>
          </form>
